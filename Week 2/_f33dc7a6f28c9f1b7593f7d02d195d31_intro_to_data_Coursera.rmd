...
# Delta and United Airlines have IQR of 22, so these two carriers has the highest IQR of arrival delays.
```
...
# 7 means July. So, the answer is July.
```
...
hig_med_dep<-nycflights %>%
  ...
  arrange(desc(Md))
hig_med_dep $ month[1]
# 12 means December. So, the month having highest median departure delay from an NYC airport is December.
```
...
# LGA has highest no. of time departures rate, so the answer is LGA.
